...
docker run --rm -e "DB_URL=postgresql://postgres:postgres@$GATEWAY_IP:5432/postgres" hbpmip/data-catalog-setup upgrade head
ret=$?

# Remove container if not on CircleCI
